...
public class CardadoCardActionDevelopmentOverlayV2 : MonoBehaviour
{
    enum Step
    {
        ...
        ExecutionerTarget, ExecutionerEffects, SpecialArtistChoice, SpecialArtistDie, SpecialArtistRerollDecision,
        SpecialSoldierChoice, SpecialSoldierTarget, SpecialSoldierDie, SpecialSoldierAllDie, SpecialCollectorChoice,
        SpecialCollectorTake, SpecialCollectorPlay, SpecialBodyguardChoice, SpecialBodyguardTarget,
        SpecialMirrorChoice, SpecialMirrorTarget, SpecialMirrorOwnDie, SpecialMirrorTargetDie,
    ...
    class Effect
    {
        ...
        public int owner, target = -1, die = -1;
        public string key;
    }
    ...
    Step step;
    bool visible;
    int pi = -1, ti = -1, si = -1, di = -1, mod;
    int trackedHand = -1, collectorPos;
    int specialArtistRerolls;
    int specialArtistDieIndex = -1;
    CardInstance active;
    string execMsg;
    string stolenNotice;

    readonly HashSet<int> blocked = new HashSet<int>();
    readonly HashSet<int> playedCard = new HashSet<int>();
    readonly HashSet<int> protectedPlayers = new HashSet<int>();
    readonly HashSet<string> protectedDice = new HashSet<string>();
    readonly HashSet<string> playedDice = new HashSet<string>();
    readonly Dictionary<string, int> modifierOriginal = new Dictionary<string, int>();
    readonly List<Effect> effects = new List<Effect>();
    readonly List<Effect> execEffects = new List<Effect>();
    readonly List<int> collectorOpp = new List<int>();
    readonly List<CardInstance> collectorPool = new List<CardInstance>();
    ...
    void OnEnable()
    {
        if (gm == null) gm = FindFirstObjectByType<CardadoGameManager>();
        if (gm == null) return;
        gm.CardActionRequested += CardRequested;
        gm.HandTurnStarted += HandStarted;
        gm.DiePlayed += DiePlayed;
        gm.RoundResolutionCompleted += RoundEnded;
    }
    ...
    void CardRequested(CardadoPlayerState p, CardadoCardActionRequestType req)
    {
        pi = Index(p);
        if (pi < 0) return;
        visible = true;
        ...
        step = req == CardadoCardActionRequestType.ChooseArtistDie ? Step.ArtistDie : Step.Cards;

        if (step == Step.Cards && (p.hand.cardsInHand.Count == 0 || blocked.Contains(pi)))
        {
            visible = false;
            step = Step.None;
    ...
    void HandStarted(CardadoPlayerState p, int hand, int starter)
    {
        if (hand != trackedHand)
        {
            trackedHand = hand;
    ...
    void DiePlayed(CardadoPlayerState p, int d, int v)
    {
        int x = Index(p);
        playedDice.Add(Key(x, d));
    }

    void ClearHandBodyguard()
    {
        for (int i = effects.Count - 1; i >= 0; i--)
            if (effects[i].type == EffectType.SpecialBodyguardHand)
                RemoveEffectObj(effects[i], true);
    }
    ...
    void OnGUI()
    {
        ...
        if (!string.IsNullOrEmpty(stolenNotice)) DrawStolenNotice();
        if (!visible || pi < 0) return;

        switch (step)
        {
            case Step.Cards: Cards(); break;
            case Step.DieAfterSkip: Dice("CHOOSE DIE", "You skipped the card. You can still go back.", pi); break;
            case Step.ArtistDie: Dice("ARTIST", "Choose one of your dice to reroll.", pi); break;
            case Step.SoldierTarget: Targets("SOLDIER", "Choose an opponent.", false, false, Step.Cards); break;
            case Step.SoldierDie: Dice("SOLDIER", "Choose a die from " + gm.Players[ti].playerId, ti); break;
            case Step.CollectorTarget: Targets("COLLECTOR", "Choose an opponent.", false, false, Step.Cards); break;
            case Step.CollectorCard: CollectorCards(); break;
            case Step.BodyguardDie: Dice("BODYGUARD", "Choose one of your dice to protect.", pi); break;
            case Step.MirrorTarget: Targets("MIRROR", "Choose an opponent.", false, false, Step.Cards); break;
            case Step.MirrorOwnDie: Dice("MIRROR", "Choose your die.", pi); break;
            case Step.MirrorTargetDie: Dice("MIRROR", "Choose " + gm.Players[ti].playerId + "'s die.", ti); break;
            case Step.ModifierDirection: ModifierDirection(); break;
            case Step.ModifierTarget: ModifierTarget(); break;
            ...
            case Step.SpecialSoldierDie: Dice("SPECIAL SOLDIER", "Choose a die from " + gm.Players[ti].playerId, ti); break;
            case Step.SpecialSoldierAllDie: SpecialSoldierAll(); break;
            case Step.SpecialCollectorChoice: Choice("SPECIAL COLLECTOR", new[] { "TAKE ONE CARD FROM EACH PLAYER", "DRAW 3 CARDS" }, ChooseSpecialCollector, Step.Cards); break;
            case Step.SpecialCollectorTake: SpecialCollectorTake(); break;
            case Step.SpecialCollectorPlay: SpecialCollectorPlay(); break;
            case Step.SpecialBodyguardChoice: Choice("SPECIAL BODYGUARD", new[] { "PROTECT ALL DICE OF ONE PLAYER", "PROTECT ALL DICE PLAYED THIS HAND" }, ChooseSpecialBodyguard, Step.Cards); break;
            case Step.SpecialBodyguardTarget: Targets("SPECIAL BODYGUARD", "Choose the player to protect.", true, true, Step.SpecialBodyguardChoice); break;
            case Step.SpecialMirrorChoice: Choice("SPECIAL MIRROR", new[] { "EXCHANGE YOUR DIE WITH AN OPPONENT", "EXCHANGE TWO OPPONENTS' DICE" }, ChooseSpecialMirror, Step.Cards); break;
            case Step.SpecialMirrorTarget: Targets("SPECIAL MIRROR", "Choose an opponent.", false, false, Step.SpecialMirrorChoice); break;
            case Step.SpecialMirrorOwnDie: Dice("SPECIAL MIRROR", "Choose your die.", pi); break;
            case Step.SpecialMirrorTargetDie: Dice("SPECIAL MIRROR", "Choose " + gm.Players[ti].playerId + "'s die.", ti); break;
            case Step.SpecialMirrorFirstOpponent: Pair("SPECIAL MIRROR", "Choose the first opponent.", true); break;
            case Step.SpecialMirrorFirstDie: Dice("SPECIAL MIRROR", "Choose " + gm.Players[ti].playerId + "'s die.", ti); break;
            case Step.SpecialMirrorSecondOpponent: Pair("SPECIAL MIRROR", "Choose the second opponent.", false); break;
            case Step.SpecialMirrorSecondDie: Dice("SPECIAL MIRROR", "Choose " + gm.Players[si].playerId + "'s die.", si); break;
            case Step.SpecialExecutionerTarget: Targets("SPECIAL EXECUTIONER", "Choose a player who must discard their entire hand.", false, true, Step.Cards); break;
            case Step.JokerTarget: Targets("JOKER", "Choose whose die to flip.", true, false, Step.Cards); break;
            case Step.JokerDie: Dice("JOKER", "Choose the die to flip.", ti); break;
            case Step.GordonChoice: Choice("GORDON ROBLEYS", new[] { "SPECIAL ARTIST", "SPECIAL SOLDIER", "SPECIAL COLLECTOR", "SPECIAL BODYGUARD" }, ChooseGordon, Step.Cards); break;
        }
    }

    void DrawEffects()
    {
        Rect r = new Rect(15, 15, 420, Mathf.Min(Screen.height - 30, 90 + effects.Count * 38));
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 15, r.y + 10, r.width - 30, 30), "ACTIVE CARD EFFECTS", title);
        float y = r.y + 50;
        foreach (var e in effects)
        {
            GUI.Label(new Rect(r.x + 15, y, r.width - 30, 30), EffectText(e), small);
            y += 38;
        }
    }

    void DrawStolenNotice()
    {
        float width = 620f;
        Rect r = new Rect((Screen.width - width) * 0.5f, 10f, width, 48f);
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 10f, r.y + 8f, r.width - 20f, 32f), stolenNotice, small);
    }

    void Cards()
    {
        CardadoPlayerState p = gm.Players[pi];
        Rect r = Box(900, 430);
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 20, r.y + 20, 860, 45), p.playerId + " — CARD ACTION", title);
        GUI.Label(new Rect(r.x + 20, r.y + 70, 860, 30), "Play a card before choosing a die, or skip.", GUI.skin.label);
        float bw = 145, g = 12;
        float total = p.hand.cardsInHand.Count * bw + Mathf.Max(0, p.hand.cardsInHand.Count - 1) * g;
        float x = r.x + (900 - total) / 2f;
    ...
    void Dice(string head, string text, int p)
    {
        ...
        for (int d = 0; d < s.dice.Count; d++)
        {
            ...
            if (GUI.Button(q, "Die " + (d + 1) + status, button)) Die(d);
            GUI.enabled = true;
            DrawValue(q, p, d);
            shown++;
        }
    ...
    void DrawValue(Rect q, int p, int d)
    {
        string k = Key(p, d);
        GUIStyle st = value;
        if (modifierOriginal.ContainsKey(k)) st = gm.Players[p].dice[d] > modifierOriginal[k] ? green : red;
        GUI.Label(new Rect(q.x, q.y + 45, q.width, 30), gm.Players[p].dice[d].ToString(), st);
    }

    void Targets(string head, string text, bool self, bool ignore, Step back)
    {
        Rect r = Box(850, 370);
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 20, r.y + 20, 810, 45), head, title);
        GUI.Label(new Rect(r.x + 20, r.y + 70, 810, 35), text, GUI.skin.label);
        float x = r.x + 25;
        for (int i = 0; i < gm.Players.Count; i++)
        {
            if (!self && i == pi) continue;
            GUI.enabled = ignore || CanPlayer(i);
            if (GUI.Button(new Rect(x, r.y + 125, 145, 75), gm.Players[i].playerId + "\nChips: " + gm.Players[i].chips, button)) Target(i);
            GUI.enabled = true;
            x += 160;
        }
    ...
    void Pair(string head, string text, bool first)
    {
        ...
        for (int i = 0; i < gm.Players.Count; i++)
        {
            if (i == pi || (!first && i == ti) || protectedPlayers.Contains(i)) continue;
            if (GUI.Button(new Rect(x, r.y + 125, 145, 75), gm.Players[i].playerId, button))
            {
                if (first) { ti = i; step = Step.SpecialMirrorFirstDie; }
                else { si = i; step = Step.SpecialMirrorSecondDie; }
            }
            x += 160;
        }
    ...
    void CollectorCards()
    {
        ...
        Rect r = Box(820, 430);
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 20, r.y + 20, 780, 45), "COLLECTOR — STEAL FROM " + p.playerId, title);
        for (int i = 0; i < p.hand.cardsInHand.Count; i++)
        {
            var c = p.hand.cardsInHand[i];
    ...
    void ModifierDirection()
    {
        Rect r = Box(700, 360);
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 20, r.y + 20, 660, 45), "MODIFIER", title);
        GUI.Label(new Rect(r.x + 20, r.y + 70, 660, 30), "Choose +1 or -1. You can still go back.", GUI.skin.label);
        if (GUI.Button(new Rect(r.x + 50, r.y + 125, 280, 70), "+1", button)) { mod = 1; step = Step.ModifierTarget; }
        if (GUI.Button(new Rect(r.x + 370, r.y + 125, 280, 70), "-1", button)) { mod = -1; step = Step.ModifierTarget; }
        if (GUI.Button(new Rect(r.x + 50, r.y + 225, 600, 55), "BACK", button)) BackCards();
    }
    ...
    void ModifierTarget()
    {
        Rect r = Box(900, 520);
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 20, r.y + 20, 860, 45), "MODIFIER " + (mod > 0 ? "+1" : "-1") + " — CHOOSE DIE", title);
        float y = r.y + 105;
        for (int p = 0; p < gm.Players.Count; p++)
        {
            GUI.Label(new Rect(r.x + 20, y, 140, 30), gm.Players[p].playerId, GUI.skin.label);
            float x = r.x + 170;
            for (int d = 0; d < gm.Players[p].dice.Count; d++)
            {
                if (!IsTargetableDie(p, d)) continue;
                bool ok = (mod > 0 && gm.Players[p].dice[d] < 6) || (mod < 0 && gm.Players[p].dice[d] > 1);
                ok &= CanAffect(p, d);
                GUI.enabled = ok;
                Rect q = new Rect(x, y - 5, 90, 70);
                if (GUI.Button(q, "Die " + (d + 1) + (IsCurrentHandPlayed(p, d) ? "\nPLAYED" : "\nAVAILABLE"), button)) Modifier(p, d);
                GUI.enabled = true;
                DrawValue(q, p, d);
                x += 105;
            }
        ...
        if (GUI.Button(new Rect(r.x + 20, r.y + 455, 860, 50), "BACK", button)) step = Step.ModifierDirection;
    }

    void Choice(string head, string[] opts, Action<int> pick, Step back)
    {
        Rect r = Box(820, 390);
        GUI.Box(r, "", box);
        GUI.Label(new Rect(r.x + 20, r.y + 20, 780, 45), head, title);
        for (int i = 0; i < opts.Length; i++) if (GUI.Button(new Rect(r.x + 35, r.y + 90 + i * 75, 750, 60), opts[i], button)) pick(i);
    ...
    void SpecialArtistRerollDecision()
    {
        ...
        bool canReroll = specialArtistRerolls < 3;
        GUI.enabled = canReroll;
        string label = canReroll ? "REROLL AGAIN" : "FINAL REROLL";
        if (GUI.Button(new Rect(r.x + 395, r.y + 165, 330, 65), label, button)) ContinueSpecialArtistReroll();
        GUI.enabled = true;

        if (specialArtistRerolls >= 3)
            GUI.Label(new Rect(r.x + 35, r.y + 255, 690, 35), "Third reroll is final. The current value must be kept.", GUI.skin.label);
    }
    ...
    void Die(int d)
    {
        switch (step)
        {
            ...
            case Step.ArtistDie:
                ...
                Commit();
                break;
            case Step.SoldierDie:
                if (CanAffect(ti, d)) { Reroll(ti, d, 1); Commit(); }
                break;
            case Step.BodyguardDie:
                NormalBodyguard(d);
                break;
            case Step.MirrorOwnDie:
                di = d;
                step = Step.MirrorTargetDie;
                break;
            case Step.MirrorTargetDie:
                if (CanAffect(ti, d)) { Exchange(pi, di, ti, d); Commit(); }
                break;
            case Step.SpecialArtistDie:
                ...
                break;
            case Step.SpecialSoldierDie:
                if (CanAffect(ti, d)) { Reroll(ti, d, 1); Commit(); }
                break;
            case Step.SpecialMirrorOwnDie:
                di = d;
                step = Step.SpecialMirrorTargetDie;
                break;
            case Step.SpecialMirrorTargetDie:
                if (CanAffect(ti, d)) { Exchange(pi, di, ti, d); Commit(); }
                break;
            case Step.SpecialMirrorFirstDie:
                di = d;
                step = Step.SpecialMirrorSecondOpponent;
                break;
            case Step.SpecialMirrorSecondDie:
                if (CanAffect(si, d)) { Exchange(ti, di, si, d); Commit(); }
                break;
            case Step.JokerDie:
                if (CanAffect(ti, d)) { gm.Players[ti].dice[d] = 7 - gm.Players[ti].dice[d]; Commit(); }
                break;
        }
    }
    ...
    void SpecialArtistRerollDecision()
    {
        ...
        {
            if (GUI.Button(new Rect(r.x + 35, r.y + 165, 330, 65), "KEEP THIS VALUE", button)) FinishSpecialArtistReroll();
            if (GUI.Button(new Rect(r.x + 395, r.y + 165, 330, 65), "REROLL AGAIN", button)) ContinueSpecialArtistReroll();
        }
        else
        {
            GUI.Label(new Rect(r.x + 35, r.y + 165, 690, 35), "Third reroll is final. This value must be kept.", GUI.skin.label);
            if (GUI.Button(new Rect(r.x + 35, r.y + 220, 690, 60), "CONTINUE", button)) FinishSpecialArtistReroll();
        }
    }

    void ContinueSpecialArtistReroll()
    {
        if (specialArtistRerolls >= 3) return;
    ...
    {
        Debug.Log("[Cardado] Special Artist finalized " + gm.Players[pi].playerId + " die " + (specialArtistDieIndex + 1) + " at " + gm.Players[pi].dice[specialArtistDieIndex] + " after " + specialArtistRerolls + " reroll(s).");
        specialArtistDieIndex = -1;
        specialArtistRerolls = 0;
        After();
    }

    void Reroll(int p, int d, int times)
    {
        if (!CanAffect(p, d) && !(p == pi && (step == Step.ArtistDie || step == Step.SpecialArtistDie))) return;
        for (int i = 0; i < times; i++) gm.Players[p].dice[d] = UnityEngine.Random.Range(1, 7);
    }

    void Exchange(int a, int ad, int b, int bd)
    {
        if (a < 0 || b < 0 || ad < 0 || bd < 0) return;
        if (!CanAffect(a, ad) || !CanAffect(b, bd)) return;
        int av = gm.Players[a].dice[ad];
        gm.Players[a].dice[ad] = gm.Players[b].dice[bd];
        gm.Players[b].dice[bd] = av;
    }

    void Collector(CardInstance c)
    {
        if (c == null || c.data == null) return;
        string cardName = c.data.id + " (" + c.data.cardType + (c.data.isBlankCard ? ", Blank" : "") + ")";
        Debug.Log("[Cardado] COLLECTOR: " + gm.Players[pi].playerId + " stole " + cardName + " from " + gm.Players[ti].playerId + ".");
        stolenNotice = "STOLEN CARD: " + cardName + " from " + gm.Players[ti].playerId;
        gm.Players[ti].hand.RemoveCard(c);
        CommitCardOnly();
        active = c;
        RouteStolen(c);
    }

    void Modifier(int p, int d)
    {
        if (!CanAffect(p, d)) return;
        int v = gm.Players[p].dice[d];
        if ((mod > 0 && v >= 6) || (mod < 0 && v <= 1)) return;
        string k = Key(p, d);
        if (modifierOriginal.ContainsKey(k)) return;
        modifierOriginal[k] = v;
        gm.Players[p].dice[d] = v + mod;
        Persistent(new Effect { type = EffectType.Modifier, card = active, owner = pi, target = p, die = d, key = k });
    }

    void NormalBodyguard(int d)
    {
        string k = Key(pi, d);
        if (protectedDice.Contains(k)) return;
        protectedDice.Add(k);
        Persistent(new Effect { type = EffectType.BodyguardDie, card = active, owner = pi, target = pi, die = d, key = k });
    }
    ...
    void SpecialSoldier(int t)
    {
        if (!CanPlayer(t)) return;
        for (int d = 0; d < gm.Players[t].dice.Count; d++) if (CanAffect(t, d)) gm.Players[t].dice[d] = UnityEngine.Random.Range(1, 7);
        Commit();
    }

    void ChooseSpecialCollector(int o)
    {
        if (o == 1)
        {
            CommitCardOnly();
    ...
    void TakeCollector(int i)
    {
        int p = collectorOpp[collectorPos];
        if (i < 0 || i >= gm.Players[p].hand.cardsInHand.Count) return;
        var c = gm.Players[p].hand.cardsInHand[i];
    ...
    void ChooseSpecialBodyguard(int o)
    {
    ...
    {
        if (step == Step.DieAfterSkip) return gm.IsDieAvailable(p, d) && p == pi;
        if (!IsTargetableDie(p, d)) return false;
        if (protectedPlayers.Contains(p)) return false;
        return CanAffect(p, d);
    }

    bool IsTargetableDie(int p, int d)
    {
        if (p < 0 || p >= gm.Players.Count) return false;
        if (d < 0 || d >= gm.Players[p].dice.Count) return false;
        return gm.IsDieAvailable(p, d) || playedDice.Contains(Key(p, d));
    }

    bool IsCurrentHandPlayed(int p, int d) => playedDice.Contains(Key(p, d));

    bool IsHandBodyguardActive()
    {
        foreach (var e in effects) if (e.type == EffectType.SpecialBodyguardHand) return true;
        return false;
    }

    bool IsProtectedDie(int p, int d)
    {
        string key = Key(p, d);
        return protectedDice.Contains(key) || (IsCurrentHandPlayed(p, d) && IsHandBodyguardActive());
    }

    bool CanAffect(int p, int d) => IsTargetableDie(p, d) && CanPlayer(p) && !IsProtectedDie(p, d);
    ...
    string EffectText(Effect e)
    {
        string owner = e.owner >= 0 && e.owner < gm.Players.Count ? gm.Players[e.owner].playerId : "?";
        switch (e.type)
        {
            case EffectType.Modifier: return (e.card.data.canAdd ? "+1" : "-1") + " — " + owner + " → " + gm.Players[e.target].playerId + " Die " + (e.die + 1);
            case EffectType.BodyguardDie: return "BODYGUARD — " + owner + " Die " + (e.die + 1);
            case EffectType.SpecialBodyguardPlayer: return "SPECIAL BODYGUARD — " + owner + " protects " + gm.Players[e.target].playerId;
            default: return "SPECIAL BODYGUARD — " + owner + " protects all dice played this hand";
        }
    }

    void RemoveEffectObj(Effect e, bool discard)
    {
        if (e == null) return;
        effects.Remove(e);
        if (discard && e.card != null) gm.DiscardResolvedCard(e.card);
    }
    ...
    int Index(CardadoPlayerState p)
    {
        for (int i = 0; i < gm.Players.Count; i++) if (gm.Players[i] == p) return i;
        return -1;
    }
    ...
    void Styles()
    {
        ...
        green = new GUIStyle(value); green.normal.textColor = Color.green;
        red = new GUIStyle(value); red.normal.textColor = Color.red;
